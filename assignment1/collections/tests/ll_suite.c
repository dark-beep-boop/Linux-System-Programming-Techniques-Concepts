...
START_TEST(test_ll_push_7_head_and_iterate_in_for_each_loop)
{
  ll_t *ll = NULL;
  ll_node_t *node = NULL;
  ...
  LL_FOR_EACH_BEGIN(ll, node)
  {
    ck_assert(ll_node_get(node));
  }
  LL_FOR_EACH_END ll_destroy(ll);
}
END_TEST
...
ll_suite(void)
{
  ...
  tcase_add_test(tc_foreach, test_ll_push_7_head_and_iterate_in_for_each_loop);
